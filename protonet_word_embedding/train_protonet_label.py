...
from model.word_embedding import wordEmbTransformer as label_net
...
def parse_args():
    ...
    parser.add_argument("--gpu_id", default="1",  help="gpu的id")
...
def train(model, model_label, train_loader, optimizer, criterion, print_period, args):
    model.train()
    total_loss = []
    label_nbr = 0 # 标签总数
    eq_nbr = 0 # 预测正确的标签数
    ...
    for batch_idx, (feature, label) in enumerate(train_loader):
        ...
        # print("-----------------", one_hot_labels.shape) # 10 * 10 * 1

        label_feature, lambda_k = model_label(one_hot_labels) 
        # print("-----------------", label_feature.shape) # 10 * 512 * 1
        # print("-----------------", lambda_k.shape) # 10 * 1
        # print("------------------", proto.shape) # 10 * 512
        ...
        # print(proto_new.shape) # 10 * 512
        # print(proto.shape) # 
        
        
        logits = euclidean_metric(model(data_query), proto_new) # (query_shot, n_cls)，查询集样本和每个类别原型的距离，距离越小越好，所以logits为负数
...
def valid(model, model_label, valid_loader, criterion, args):
    model.eval()
...
def main(args):
    ...
    model_label = label_net(args.n_cls)
    model.to(device)
    ...
    for epoch in range(args.epochs):
        ...
        train_acc, train_loss = train(model, model_label, trainLoader, optimizer, loss_func, args.print_period, args)
        val_acc, val_loss, val_label, val_predict = valid(model, model_label, testLoader, loss_func, args)
        scheduler.step()
