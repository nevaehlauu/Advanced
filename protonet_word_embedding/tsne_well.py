...
def parse_args():
    ...
    parser.add_argument("--pretrained_filepath", default="log/resnet_log/12_23_2024__21_49_40/best_epoch_model.pth", help="预训练模型位置")
    ...
    parser.add_argument("--support", default=50, help="每个类别中选择的support样本数量")
    parser.add_argument("--query", default=10, help="每个类别中选择的query样本数量")
...
def main(args):
    ...
    features = []
    labels = []
    for batch_ind, (feature, label) in enumerate(testLoader):
        with torch.no_grad():
    ...
    # for label in np.unique(labels):
    #     indices = np.where(labels == label)
    #     ax.scatter(features_tsne[indices, 0], features_tsne[indices, 1], features_tsne[indices, 2], 
    #                s=90, edgecolor='white', linewidth=1,
    #                label=f'Class {label}', color=color_map[label], alpha=1)
