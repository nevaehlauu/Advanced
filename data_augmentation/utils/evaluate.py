...
def common_forward(model, batch, device, criterion, features_name: list):
    """
    前向传播复用代码
    : param model: 网络模型
    : param batch: 批尺寸大小
    ...
    this_batch = sample_to_device(batch, device)
...
def evaluate(model, data_loader, criterion, features_name: list):
    """
    评估模型
    :param model: 网络模型
