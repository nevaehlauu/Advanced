...
from config.darts_config import search_parse_args
# from data_228.readtxt_sample import dataloader, dataloader_split
...
def train(train_loader, valid_loader, model, architect, optimizer, criterion, lr, args):
    total_loss = []
    label_nbr = 0
    eq_nbr = 0
    ...
    for batch_idx, (features, label) in enumerate(train_loader):
        ...
        val_features, val_label = next(iter(valid_loader))
        val_features, val_label = val_features.to(cur_device), val_label.to(cur_device)

        features = features.detach().requires_grad_(False)
        label = label.long().detach().requires_grad_(False)
        val_features = val_features.detach().requires_grad_(False)
        val_label = val_label.long().detach().requires_grad_(False)

        # 更新架构权重alpha，unrolled为True时就是用论文的公式进行alpha的更新
        architect.step(features, label, val_features, val_label, lr, optimizer, args.unrolled)
...
def main(args):
    ...
    args.save = save_log("search", args)
    ...
    model = Network(args.init_channels, len(label_name), args.layers, criterion, device)
    model.to(device)
    ...
    for epoch in range(args.epochs):
        ...
        train_loss, train_acc = train(trainLoader, validLoader, model, architect, optimizer, criterion, lr, args)
