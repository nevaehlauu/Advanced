...
from dataloader_aug import setup_dataloaders
...
from config.get_config import get_train_cfg, cfg2dict
from utils.utils import print_block, set_seeds, load_model_params, printcolor, save_train_val_fig, get_classes_map
...
def parse_args():
    ...
    parser.add_argument('--datasets_cfg_file', type=str, default="../config/train_data.yaml", help='存放预训练模型等文件的路径')
    parser.add_argument('--model_config_file', type=str, default="../config/training_data_config.yaml", help='存放数据以及模型配置文件等的路径')
...
def main(args):
    ...
    classification_name = cfg.model.params.generic.classification_name

    if classification_name == "储层划分":
        train_classes = ["0", "1"]
        val_classes = ["0", "1"]
    else: 
    ...
    # 获取训练集
    # train_dataset, train_loader, val_dataset, val_loader = setup_dataloaders(
    ...
    model = Net(features_name=features_name, classes=len(train_classes))
    ...
    if cfg.model.log_dir_name is None or cfg.model.log_dir_name == "":
        ...
        date_time = cfg.model.author + '_' + classification_name + "_resnet_" + date_time + "_" + str(cfg.model.params.generic.slice_length)  # 模型名字加时间吧
    ...
    for epoch in range(cfg.arch.epochs):
        ...
        save_train_val_fig(result_dict['trainacc'], result_dict['valacc'], 'train_acc', 'val_acc', "train and val accuracy", "epochs", "accuracy", str(Path(cfg.model.log_dir_path) / "train_val_acc.png"))
        save_train_val_fig(result_dict['trainloss'], result_dict['valloss'], 'train_loss', 'val_loss', "train and val loss", "epochs", "loss", str(Path(cfg.model.log_dir_path) / "train_val_loss.png"))
