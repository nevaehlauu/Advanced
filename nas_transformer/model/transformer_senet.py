...
class Embeddings(nn.Module):
    ...
    def __init__(self, well_len, patch_size, in_channel, d_model, dropout=0.1):
        ...
        self.embedding = SENet18(in_channels=self.in_channel, classes=self.patch_size)
        # self.patch_embedding = nn.Conv1d(in_channels=512, out_channels=self.d_model, kernel_size=self.patch_size, stride=self.patch_size)
        # 对测井片段进行变换，获取测井块，且每一个测井块映射到d_model维度
        # self.patch_embedding = nn.Conv1d(in_channels=self.in_channel, out_channels=self.d_model, kernel_size=self.patch_size, stride=self.patch_size)


    def forward(self, x):
        x = self.embedding(x)
        # print("---senet: x.shape---", x.shape)
        # x = self.patch_embedding(x) # (batchsize, d_model, well_len / patch_size)
...
def Net(args):
    ...
    return make_model(well_len=args.slice_length, patch_size=8, in_channel=args.in_channel, tgt_vovab=args.classes_num, cell_num = args.cell_num) # 六条测井曲线
